...
<form class="mx-20" action="{{ url('allAdmin',['adminId' => $admin->id]) }}"
    method="POST">
    @csrf
    @method('PUT')
    <div class="mt-5">
        <p class="py-3">Name</p>
        <input value="{{$admin->name}}" type="text" name="name" placeholder="Enter Name"
            class="input input-bordered w-full " />
    </div>

    <div class="mt-5">
        <p class="py-3">Email</p>
        <input value="{{$admin->email}}" type="email" name="email" placeholder="Enter Email"
            class="input input-bordered w-full " />
    </div>
    <div class="mt-5">
        <p class="py-3">Phone Number</p>
        <input value="{{$admin->phone_number}}" type="text" name="phone_number" placeholder=""
            class="input input-bordered w-full " />
    </div>
    <div class="mt-5">
        ...
        <select name="role" class="select select-bordered w-full ">
            ...
            @endif
            @if($adminData && $adminData->role == 'admin_one' || $adminData->role == 'admin_two' ||
